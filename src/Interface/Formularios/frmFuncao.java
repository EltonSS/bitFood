...
import Interface.Formularios.Registrar.frmFuncaoRegistrar;
...
import Interface.Formularios.Alterar.FrmFuncaoAlterar;
import javax.swing.JDialog;

public class frmFuncao extends javax.swing.JDialog {

    private final JDialog parente;
    ConectaBanco conBanco = new ConectaBanco();
    crudFuncao crudFrm = new crudFuncao();
    TratamentoCampos tratCampo = new TratamentoCampos();
    int xx, yy, id_func, num = 1;
    String desc_func, status_func;

    public frmFuncao(java.awt.Frame parent, boolean modal, JDialog p) {
    ...
    private void initComponents() {
        ...
        jLabel1.setText("Funções Usuários");

        jLabel2.setIcon(new javax.swing.ImageIcon(getClass().getResource("/Imagem/FachadasFormularios/Funcoes.png"))); // NOI18N
    ...
    public void consultarRegistro() {
        ...
        if (num == 0) {
            preencherTabela(" select * from tab_funcao where desc_func like '%" + txtConsultarRegistro.getText() + "%' and status_func = 'ativo' order by desc_func asc");
            num = 1;
        } else {
            if (num == 1) {
                preencherTabela(" select * from tab_funcao where id_func = '" + txtConsultarRegistro.getText() + "' and status_func = 'ativo' ");
            }
        }
    }
    ...
    public void atualizaTabela() {
        ...
        preencherTabela(" select * from tab_funcao where status_func = 'ativo'");
    }

    public void preencherTabela(String SQL) {

        ArrayList dados = new ArrayList();
        String[] Colunas = new String[]{"Funções"};
        conBanco.conexao();
        conBanco.executaSQL(SQL);
        try {
            conBanco.rs.first();
            do {
                dados.add(new Object[]{conBanco.rs.getString("desc_func")});
...
//            JOptionPane.showMessageDialog(null, "Erro ao tentar exibir a permissão:" + ex);
    ...
    private void btNovoRegistroActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btNovoRegistroActionPerformed
        ...
        frmFuncaoRegistrar frmFuncReg = new frmFuncaoRegistrar(null, true, this);
        frmFuncReg.setVisible(true);
    ...
    private void btDeletarRegistroActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btDeletarRegistroActionPerformed
        ...
        if (opcao_escolhida == JOptionPane.YES_OPTION) {
            String descFunc = "" + tabelaForm.getValueAt(tabelaForm.getSelectedRow(), 0);
            crudFrm.setDescFunc(descFunc);
            crudFrm.ConsultarFuncao();
            id_func = crudFrm.getIdFunc();
            status_func = "não ativo";
            crudFrm.DesativarFuncao(id_func, status_func);
            atualizaTabela();
        }
    }//GEN-LAST:event_btDeletarRegistroActionPerformed

    public void chamarFormularioAtualizarFuncao() {

        String descFunc = "" + tabelaForm.getValueAt(tabelaForm.getSelectedRow(), 0);
        crudFrm.setDescFunc(descFunc);
        crudFrm.ConsultarFuncao();
        desc_func = crudFrm.getDescFunc();
        status_func = crudFrm.getStatusFunc();
        id_func = crudFrm.getIdFunc();
        FrmFuncaoAlterar frmPerm = new FrmFuncaoAlterar(null, true, desc_func, status_func, id_func, this);
    ...
    private void btEditarRegistroActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btEditarRegistroActionPerformed
        ...
        chamarFormularioAtualizarFuncao();
