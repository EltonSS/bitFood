...
import Interface.Formularios.Registrar.frmCategoriaProdutoRegistrar;
...
import Interface.Formularios.Alterar.FrmCategoriaProdutoAlterar;
import Interface.Formularios.Alterar.FrmUnidadeMedidaAlterar;
import javax.swing.JDialog;

public class frmCategoriaProduto extends javax.swing.JDialog {

    private final JDialog parente;
    ConectaBanco conBanco = new ConectaBanco();
    crudCategoriaProduto crudCategProd = new crudCategoriaProduto();
    TratamentoCampos tratCampo = new TratamentoCampos();
    int xx, yy, num = 1, id_Cat_Prod;
    String desc_Cat_Prod, status_Cat_Prod;

    public frmCategoriaProduto(java.awt.Frame parent, boolean modal, JDialog p) {
    ...
    private void initComponents() {
        ...
        jLabel2.setIcon(new javax.swing.ImageIcon(getClass().getResource("/Imagem/FachadasFormularios/Categoria Produto.png"))); // NOI18N
    ...
    public void consultarRegistro() {
        ...
        if (num == 0) {
            preencherTabela(" select * from tab_categoria_produto where desc_cat_prod like '%" + txtConsultarRegistro.getText() + "%' and status_cat_prod = 'ativo' order by desc_cat_prod asc");
            num = 1;
        } else {
            if (num == 1) {
                preencherTabela(" select * from tab_categoria_produto where id_cat_prod = '" + txtConsultarRegistro.getText() + "' and status_cat_prod = 'ativo' ");
            }
        }
    }
    ...
    public void atualizaTabela() {
        ...
        preencherTabela(" select * from tab_categoria_produto where status_cat_prod = 'ativo'");
    }

    public void preencherTabela(String SQL) {

        ArrayList dados = new ArrayList();
        String[] Colunas = new String[]{"Descrição Categoria Produto"};
        conBanco.conexao();
        conBanco.executaSQL(SQL);
        try {
            conBanco.rs.first();
            do {
                dados.add(new Object[]{conBanco.rs.getString("desc_cat_prod")});
...
//            JOptionPane.showMessageDialog(null, "Erro ao tentar exibir a permissão:" + ex);
    ...
    private void btNovoRegistroActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btNovoRegistroActionPerformed
        ...
        frmCategoriaProdutoRegistrar frmFormReg = new frmCategoriaProdutoRegistrar(null, true, this);
    ...
    private void btDeletarRegistroActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btDeletarRegistroActionPerformed
        ...
        if (opcao_escolhida == JOptionPane.YES_OPTION) {
            String descCatProd = "" + tabelaForm.getValueAt(tabelaForm.getSelectedRow(), 0);
            crudCategProd.DesativarCategoriaProduto(descCatProd, "não ativo");
            atualizaTabela();
        }
    }//GEN-LAST:event_btDeletarRegistroActionPerformed

    public void chamarFormularioAtualizarCategoriaProduto() {

        String descCategProd = "" + tabelaForm.getValueAt(tabelaForm.getSelectedRow(), 0);
        crudCategProd.ConsultarCategoriaProduto(descCategProd);
        desc_Cat_Prod = crudCategProd.getDesc_Cat_Prod();
        status_Cat_Prod = crudCategProd.getStatus_Cat_Prod();
        id_Cat_Prod = crudCategProd.getId_Cat_Prod();
        FrmCategoriaProdutoAlterar frmCategProd = new FrmCategoriaProdutoAlterar(null, true, id_Cat_Prod, desc_Cat_Prod, status_Cat_Prod, this);
        frmCategProd.setVisible(true);
    }
    ...
    private void btEditarRegistroActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btEditarRegistroActionPerformed
        ...
        chamarFormularioAtualizarCategoriaProduto();
