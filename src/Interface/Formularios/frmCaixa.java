...
import Interface.Formularios.Registrar.frmUsuarioRegistrar;
...
public final class frmCaixa extends javax.swing.JDialog {
    ...
    int xx, yy, num = 1, id_Usu;
    String nome_Usu, login_Usu, desc_Func, desc_Prm, senha_Usu, status_Usu;

    public frmCaixa(java.awt.Frame parent, boolean modal, JDialog p) {
    ...
    private void initComponents() {
        ...
        jLabel2.setIcon(new javax.swing.ImageIcon(getClass().getResource("/Imagem/FachadasFormularios/Usuario.png"))); // NOI18N
        ...
        getContentPane().add(rSPanelShadow1, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 0, 930, 560));
    ...
    public void consultarRegistro() {
        ...
        if (num == 0) {
            preencherTabela(" select * from vw_usuario where nome_usu like '%" + txtConsultarRegistro.getText() + "%' and status_usu = 'ativo' order by nome_usu asc");
            num = 1;
        } else {
            if (num == 1) {
                preencherTabela(" select * from tab_usuario where id_usu = '" + txtConsultarRegistro.getText() + "' and status_usu = 'ativo' ");
            }
        }
    }
    ...
    public void atualizaTabela() {
        ...
        preencherTabela(" select * from vw_usuario where status_usu = 'ativo'");
    }

    public void preencherTabela(String SQL) {

        ArrayList dados = new ArrayList();
        String[] Colunas = new String[]{"Nome usuário", "Função", "Login", "Permissão"};
        conBanco.conexao();
        conBanco.executaSQL(SQL);
        try {
            conBanco.rs.first();
            do {
                dados.add(new Object[]{conBanco.rs.getString("nome_usu"), conBanco.rs.getString("desc_func"),
                    conBanco.rs.getString("login_usu"), conBanco.rs.getString("desc_prm")});
...
//            JOptionPane.showMessageDialog(null, "Erro ao tentar exibir a permissão:" + ex);
        ...
        tabelaForm.getColumnModel().getColumn(0).setPreferredWidth(291);
        tabelaForm.getColumnModel().getColumn(0).setResizable(false);
        tabelaForm.getColumnModel().getColumn(1).setPreferredWidth(211);
        tabelaForm.getColumnModel().getColumn(1).setResizable(false);
        tabelaForm.getColumnModel().getColumn(2).setPreferredWidth(175);
        tabelaForm.getColumnModel().getColumn(2).setResizable(false);
        tabelaForm.getColumnModel().getColumn(3).setPreferredWidth(225);
        tabelaForm.getColumnModel().getColumn(3).setResizable(false);
        tabelaForm.getTableHeader().setReorderingAllowed(false);
    ...
    private void btNovoRegistroActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btNovoRegistroActionPerformed
        ...
        frmUsuarioRegistrar frmFormReg = new frmUsuarioRegistrar(null, true, this);
    ...
    private void btDeletarRegistroActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btDeletarRegistroActionPerformed
        ...
        String nomUsu = "Você deseja deletar o registro:'" + tabelaForm.getValueAt(tabelaForm.getSelectedRow(), 0) + "'";
        int opcao_escolhida = JOptionPane.showConfirmDialog(null, nomUsu, "", JOptionPane.YES_NO_OPTION);
        if (opcao_escolhida == JOptionPane.YES_OPTION) {
//            String nomeUsu = "" + tabelaForm.getValueAt(tabelaForm.getSelectedRow(), 0);
            crudUsu.DesativarUsuario(nomUsu, "não ativo");
            atualizaTabela();
        }
    }//GEN-LAST:event_btDeletarRegistroActionPerformed

    public void chamarFormularioAtualizarUsuario() {

        String nomeUsu = "" + tabelaForm.getValueAt(tabelaForm.getSelectedRow(), 0);
        crudUsu.ConsultarUsuario(nomeUsu);
        id_Usu = crudUsu.getId_Usu();
        nome_Usu = crudUsu.getNome_Usu();
        login_Usu = crudUsu.getLogin_Usu();
        desc_Func = crudUsu.getDesc_Func();
        desc_Prm = crudUsu.getDesc_Prm();
        senha_Usu = crudUsu.getSenha_Usu();
        status_Usu = crudUsu.getStatus_Usu();
        FrmUsuarioAlterar frmUsu = new FrmUsuarioAlterar(null, true, id_Usu, nome_Usu, login_Usu, desc_Func, desc_Prm, senha_Usu, status_Usu, this);
    ...
    private void btEditarRegistroActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btEditarRegistroActionPerformed
        ...
        chamarFormularioAtualizarUsuario();
