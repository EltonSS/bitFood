...
import Interface.Formularios.Registrar.frmFormularioRegistrar;
...
import Interface.Formularios.Alterar.FrmFormularioAlterar;
...
public class frmFormulario extends javax.swing.JDialog {
    ...
    int xx, yy, id_frm, num = 1;
    String desc_frm, status_frm;

    public frmFormulario(java.awt.Frame parent, boolean modal, JDialog p) {
    ...
    private void initComponents() {
        ...
        jLabel2.setIcon(new javax.swing.ImageIcon(getClass().getResource("/Imagem/FachadasFormularios/Formulario.png"))); // NOI18N
    ...
    public void consultarRegistro() {
        ...
        if (num == 0) {
            preencherTabela(" select * from tab_formulario where desc_frm like '%" + txtConsultarRegistro.getText() + "%' and status_frm = 'ativo' order by desc_frm asc");
            num = 1;
        } else {
            if (num == 1) {
                preencherTabela(" select * from tab_formulario where id_frm = '" + txtConsultarRegistro.getText() + "' and status_frm = 'ativo' ");
            }
        }
    }
    ...
    public void atualizaTabela() {
        ...
        preencherTabela(" select * from tab_formulario where status_frm = 'ativo'");
    }

    public void preencherTabela(String SQL) {

        ArrayList dados = new ArrayList();
        String[] Colunas = new String[]{"Formulários"};
        conBanco.conexao();
...
//            JOptionPane.showMessageDialog(null, "Erro ao tentar exibir a permissão:" + ex);
    ...
    private void btNovoRegistroActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btNovoRegistroActionPerformed
        ...
        frmFormularioRegistrar frmFormReg = new frmFormularioRegistrar(null, true, this);
    ...
    private void btDeletarRegistroActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btDeletarRegistroActionPerformed
        ...
        if (opcao_escolhida == JOptionPane.YES_OPTION) {
            String descFrm = "" + tabelaForm.getValueAt(tabelaForm.getSelectedRow(), 0);
            crudFrm.setDescFrm(descFrm);
            crudFrm.ConsultarFormulario();
            id_frm = crudFrm.getIdFrm();
            status_frm = "não ativo";
            crudFrm.DesativarFormulario(id_frm, status_frm);
            atualizaTabela();
        }
    ...
    public void chamarFormularioAtualizarPermissao() {
        ...
        crudFrm.setDescFrm(descFrm);
        crudFrm.ConsultarFormulario();
        desc_frm = crudFrm.getDescFrm();
        status_frm = crudFrm.getStatusFrm();
        id_frm = crudFrm.getIdFrm();
        FrmFormularioAlterar frmPerm = new FrmFormularioAlterar(null, true, desc_frm, status_frm, id_frm, this);
