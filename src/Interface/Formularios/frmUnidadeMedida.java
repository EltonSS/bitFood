...
import Interface.Formularios.Registrar.frmUnidadeMedidaRegistrar;
...
public class frmUnidadeMedida extends javax.swing.JDialog {
    ...
    int xx, yy, num = 1, id_Unid_Med;
    String desc_Unid_Med, status_Unid_Med;

    public frmUnidadeMedida(java.awt.Frame parent, boolean modal, JDialog p) {
    ...
    private void initComponents() {
        ...
        jLabel2.setIcon(new javax.swing.ImageIcon(getClass().getResource("/Imagem/FachadasFormularios/Unidade Medida.png"))); // NOI18N
    ...
    public void consultarRegistro() {
        ...
        if (num == 0) {
            preencherTabela(" select * from tab_unidade_medida where desc_unid_med like '%" + txtConsultarRegistro.getText() + "%' and status_unid_med = 'ativo' order by desc_unid_med asc");
            num = 1;
        } else {
            if (num == 1) {
                preencherTabela(" select * from tab_unidade_medida where id_unid_med = '" + txtConsultarRegistro.getText() + "' and status_unid_med = 'ativo' ");
            }
        }
    }
    ...
    public void atualizaTabela() {
        ...
        preencherTabela(" select * from tab_unidade_medida where status_unid_med = 'ativo'");
    }

    public void preencherTabela(String SQL) {

        ArrayList dados = new ArrayList();
        String[] Colunas = new String[]{"Descrição unidade de medida"};
        conBanco.conexao();
        conBanco.executaSQL(SQL);
        try {
            conBanco.rs.first();
            do {
                dados.add(new Object[]{conBanco.rs.getString("desc_unid_med")});
...
//            JOptionPane.showMessageDialog(null, "Erro ao tentar exibir a permissão:" + ex);
    ...
    private void btNovoRegistroActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btNovoRegistroActionPerformed
        ...
        frmUnidadeMedidaRegistrar frmFormReg = new frmUnidadeMedidaRegistrar(null, true, this);
    ...
    private void btDeletarRegistroActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btDeletarRegistroActionPerformed
        ...
        if (opcao_escolhida == JOptionPane.YES_OPTION) {
            String descUnidMed = "" + tabelaForm.getValueAt(tabelaForm.getSelectedRow(), 0);
            crudUnidMed.DesativarUnidadeMedida(descUnidMed, "não ativo");
            atualizaTabela();
        }
    ...
    public void chamarFormularioAtualizarUnidadeMedida() {
        ...
        String pergunta = "" + tabelaForm.getValueAt(tabelaForm.getSelectedRow(), 0);
        crudUnidMed.ConsultarUnidadeMedida(pergunta);
        desc_Unid_Med = crudUnidMed.getDesc_Unid_Med();
        status_Unid_Med = crudUnidMed.getStatus_Unid_Med();
        id_Unid_Med = crudUnidMed.getId_Unid_Med();
        FrmUnidadeMedidaAlterar frmUnidMed = new FrmUnidadeMedidaAlterar(null, true, id_Unid_Med, desc_Unid_Med, status_Unid_Med, this);
